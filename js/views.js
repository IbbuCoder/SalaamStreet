...
(function () {
  ...
  function srAyahCard(i, arText, trText, tlText) {
    ...
    return '<article class="card ayah-card" id="ayah-' + n + '" data-n="' + n + '">' +
      ...
      '<button class="btn-ghost btn-sm" data-tafsir="' + n + '" type="button">' + esc(t("tafsir.button")) + "</button>" +
  ...
  function showTafsir(surah, ayah) {
    ...
    $("tafsir-body").innerHTML = '<div class="skeleton" style="min-block-size:80px"></div>';
    $("tafsir-close").onclick = function () { try { dlg.close(); } catch (e) { /* noop */ } };
    dlg.showModal();
    SS.api.tafsir(surah, ayah).then(function (r) {
      var text = (r.text || "").trim();
      $("tafsir-body").innerHTML = text
        ? '<p class="translation">' + esc(text) + "</p>"
        : '<p class="muted">' + esc(t("tafsir.none")) + "</p>";
    }).catch(function () {
      $("tafsir-body").innerHTML = '<p class="error-state">' + esc(t("common.error")) + "</p>";
    });
  }
  ...
  function hadithInit(params) {
    ...
    $("hd-back").onclick = function () { location.hash = "#/hadith"; };
  }

  function hdRenderHome() {
    $("hd-detail").hidden = true;
    $("hd-cats").hidden = false;
    var ar = SS.i18n.isAr();
    var html = "";
    for (var i = 0; i < SS.HADITH_COLLECTIONS.length; i++) {
      var c = SS.HADITH_COLLECTIONS[i];
      html += '<a class="dua-cat" href="#/hadith/' + c.id + '">' +
        '<span class="ic" aria-hidden="true">📜</span><span>' + esc(ar ? c.ar : c.en) + "</span></a>";
    }
    $("hd-cats").innerHTML = html;
  }

  function hdCard(en, ar, ref, grade) {
    return '<article class="card dua-card">' +
      (ar ? '<p class="arabic-dua" lang="ar">' + esc(ar) + "</p>" : "") +
      (en ? '<p class="translation">' + esc(en) + "</p>" : "") +
      '<p class="mt-1"><span class="badge badge-src">' + esc(t("hadith.reference")) + ": " + esc(ref) + "</span>" +
      (grade ? ' <span class="badge">' + esc(t("hadith.grade")) + ": " + esc(grade) + "</span>" : "") + "</p></article>";
  }

  function hdRef(coll, h) {
    var r = h && h.reference;
    if (r && r.book != null) return coll.cite + " — Book " + r.book + ", Hadith " + r.hadith;
  ...
  function hdRenderCollection(id, numParam) {
    ...
    $("hd-title").textContent = SS.i18n.isAr() ? coll.ar : coll.en;
    $("hd-browse").hidden = coll.bounded;
    $("hd-list").innerHTML = '<div class="skeleton" style="min-block-size:90px"></div>';
      ...
      Promise.all([SS.api.hadithEdition(coll.eng), SS.api.hadithEdition(coll.ara)])
        .then(function (res) {
          ...
          for (var j = 0; j < en.length; j++) {
            var h = en[j];
            html += hdCard(h.text, araMap[h.hadithnumber] || "", hdRef(coll, h), hdGrade(h));
          }
          $("hd-list").innerHTML = html;
        })
        .catch(function () { hdError(function () { hdRenderCollection(id, numParam); }); });
    } else {
      var num = parseInt(numParam, 10) || 1;
      $("hd-num").value = num;
      $("hd-go").onclick = function () {
        var n = parseInt($("hd-num").value, 10) || 1;
  ...
  function hdLoadOne(coll, num) {
    $("hd-list").innerHTML = '<div class="skeleton" style="min-block-size:90px"></div>';
    Promise.all([SS.api.hadithOne(coll.eng, num), SS.api.hadithOne(coll.ara, num)])
      .then(function (res) {
        var en = res[0], ar = res[1];
        if (!en && !ar) { $("hd-list").innerHTML = '<p class="empty-state">—</p>'; return; }
        $("hd-list").innerHTML = hdCard(en ? en.text : "", ar ? ar.text : "", hdRef(coll, en || ar), hdGrade(en));
      })
      .catch(function () { hdError(function () { hdLoadOne(coll, num); }); });
  }

  function hdError(retry) {
    $("hd-list").innerHTML = '<div class="error-state"><p>' + esc(t("common.error")) + "</p>" +
      '<button class="btn btn-outline btn-sm mt-1" id="hd-retry" type="button">' + esc(t("common.retry")) + "</button></div>";
    var b = $("hd-retry"); if (b) b.onclick = retry;
  }

  /* ═══════════ ISLAMIC CALENDAR ═══════════ */
  function calendarInit() {
    $("cal-hijri").textContent = t("common.loading");
    $("cal-greg").textContent = "";
    SS.api.gToH(new Date()).then(function (h) {
      var monthName = SS.i18n.isAr() ? h.month.ar : h.month.en;
      $("cal-hijri").textContent = h.day + " " + monthName + " " + h.year + " AH";
      $("cal-greg").textContent = new Date().toLocaleDateString(SS.i18n.isAr() ? "ar" : undefined,
        { weekday: "long", year: "numeric", month: "long", day: "numeric" });
      calRenderEvents(+h.month.number, +h.year);
    }).catch(function () {
      $("cal-hijri").textContent = t("common.error");
      calRenderEvents(0, 0);
    });
  }

  function calRenderEvents(curMonth, curYear) {
    var ar = SS.i18n.isAr();
    var MONTHS = ["Muharram","Safar","Rabi al-Awwal","Rabi al-Thani","Jumada al-Awwal","Jumada al-Thani",
      "Rajab","Sha'ban","Ramadan","Shawwal","Dhul-Qi'dah","Dhul-Hijjah"];
    var html = "";
    for (var i = 0; i < SS.ISLAMIC_EVENTS.length; i++) {
      var e = SS.ISLAMIC_EVENTS[i];
      var upcoming = curMonth && (e.month > curMonth || (e.month === curMonth));
      html += '<div class="time-row' + (upcoming ? " next" : "") + '">' +
        '<span class="name">' + esc(ar ? e.ar : e.en) +
        (e.note ? '<span class="ar" style="font-weight:400">' + esc(e.note) + "</span>" : "") + "</span>" +
        '<span class="t">' + e.day + " " + esc(MONTHS[e.month - 1]) + "</span></div>";
    }
    $("cal-events").innerHTML = html;
  }

  /* ═══════════ PREMIUM ═══════════ */
  function premiumInit(params) {
    var ar = SS.i18n.isAr();
    $("pr-status").hidden = !SS.premium.isActive();
    $("pr-success").hidden = !(params && params[0] === "success");
    $("pr-cancel").hidden = !(params && params[0] === "cancel");
    if (params && params[0] === "success" && !SS.premium.isActive()) {
      // Landed on success without activation → send to checkout confirm.
    }

    // Feature list
    var fhtml = "";
    for (var i = 0; i < SS.PREMIUM_FEATURES.length; i++) {
      var f = SS.PREMIUM_FEATURES[i];
      fhtml += '<div class="row" style="padding-block:5px"><span>' + f.icon + "</span><span>" +
        esc(ar ? f.ar : f.en) + '</span><span class="spacer" style="flex:1"></span>' +
        '<span class="badge' + (f.live ? "" : " badge-src") + '">' +
        esc(f.live ? t("premium.live") : t("premium.soon")) + "</span></div>";
    }
    $("pr-features").innerHTML = fhtml;

    // Plans
    var phtml = "";
    for (var j = 0; j < SS.PLANS.length; j++) {
      var p = SS.PLANS[j];
      var per = p.id === "monthly" ? t("premium.perMonth") : p.id === "yearly" ? t("premium.perYear") : t("premium.oneTime");
      phtml += '<div class="card center' + (p.best ? '" style="border-color:var(--gold)"' : '"') + ">" +
        (p.best ? '<span class="badge badge-gold">' + esc(t("premium.best")) + "</span>" : "") +
        "<h3 class='mt-1'>" + esc(ar ? p.ar : p.en) + "</h3>" +
        '<p class="countdown" style="font-size:1.8rem">$' + p.price.toFixed(2) + "</p>" +
        '<p class="tiny">' + esc(per) + "</p>" +
        '<a class="btn btn-gold btn-sm mt-1" href="#/checkout/' + p.id + '" data-i18n="premium.upgrade">' + esc(t("premium.upgrade")) + "</a></div>";
    }
    $("pr-plans").innerHTML = phtml;
  }

  /* ═══════════ CHECKOUT ═══════════ */
  function checkoutInit(params) {
    var ar = SS.i18n.isAr();
    var planId = (params && params[0]) || "yearly";
    var plan = null;
    for (var i = 0; i < SS.PLANS.length; i++) if (SS.PLANS[i].id === planId) plan = SS.PLANS[i];
    if (!plan) plan = SS.PLANS[1];

    $("co-plan").textContent = ar ? plan.ar : plan.en;
    $("co-total").textContent = "$" + plan.price.toFixed(2);
    // PayPal.me deep link with amount (opens PayPal to pay @aqmoha).
    $("co-pay").setAttribute("href", SS.PAYPAL_ME + "/" + plan.price.toFixed(2) + "USD");

    $("co-activate").onclick = function () {
      var ref = $("co-ref").value.trim();
      SS.premium.activate(plan.id, ref);
      location.hash = "#/premium/success";
    };
  }

  /* ═══════════ MEMBERSHIP ═══════════ */
  function membershipInit() {
    var active = SS.premium.isActive();
    var p = SS.store.get("premium", null);
    $("mb-status").textContent = active ? t("membership.statusActive") : t("membership.statusFree");
    $("mb-plan").textContent = active && p ? p.plan : "—";
    $("mb-since").textContent = active && p && p.since ? new Date(p.since).toLocaleDateString() : "—";
    $("mb-renews").textContent = active && p && p.expires ? new Date(p.expires).toLocaleDateString() :
      (active ? t("premium.oneTime") : "—");

    $("mb-restore").onclick = function () { location.hash = "#/premium"; };
    $("mb-cancel").onclick = function () { SS.premium.deactivate(); membershipInit(); SS.toast(t("membership.statusFree")); };
  }
